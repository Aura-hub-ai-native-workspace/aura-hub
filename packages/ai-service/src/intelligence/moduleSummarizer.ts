...
/* ── File-level summary (lightweight) ────────────────────────────── */
...
export function summarizeFile(filePath: string): FileSummary | null {
  let content: string;
  ...
  const exports = (content.match(/export\s+(?:default\s+)?(?:async\s+)?(?:function|class|const|let|var|interface|type)\s+(\w+)/g) ?? [])
    .map(e => e.replace(/export\s+(?:default\s+)?(?:async\s+)?(?:function|class|const|let|var|interface|type)\s+/, ''))
    .slice(0, 10);
  ...
  const functions = (content.match(/(?:export\s+)?(?:async\s+)?function\s+(\w+)/g) ?? [])
    .map(f => f.replace(/(?:export\s+)?(?:async\s+)?function\s+/, ''))
    .slice(0, 10);

  // Derive description from leading comments or JSDoc
...
function extractFileDescription(content: string): string {
  ...
  if (match) return (match[1] ?? match[2] ?? '').trim().slice(0, 200);
  // Check for module-level JSDoc
  const jsdoc = content.match(/\/\*\*\s*\n\s*\*\s+(.+?)\n/);
  if (jsdoc) return jsdoc[1].trim().slice(0, 200);
  return '';
}
