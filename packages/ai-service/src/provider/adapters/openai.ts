...
export class OpenAIAdapter extends BaseOpenAICompatible {
  readonly metadata = { id: 'openai', name: 'OpenAI', description: 'GPT models by OpenAI', docsUrl: 'https://platform.openai.com/api-keys' };
  protected baseUrl = 'https://api.openai.com/v1';

  detect(apiKey: string): boolean { return apiKey.startsWith('sk-'); }

  createRuntime(apiKey: string, model?: string): Runtime {
    return this.makeRuntime({ baseUrl: this.baseUrl, apiKey, defaultModel: model || 'gpt-4o' });
  }
}
