...
try {
  ...
  while (Date.now() < deadline) {
    ...
    // A task parked in review needs an explicit human accept to finish.
    const review = (record.execution?.dag?.nodes ?? []).filter((n) => n.status === 'review');
  ...
  if (expectedNodes.size === 0) {
    ...
    check('7. the expected Workspace node became active',
      lit.some((id) => expectedNodes.has(id)),
      `expected=${[...expectedNodes].join(',')} lit=${[...litNodes].join(',') || 'none'}`);
